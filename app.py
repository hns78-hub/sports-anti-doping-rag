...
if active_provider == "Google Gemini":
    ...
    selected_emb_model = st.sidebar.selectbox(
        ...
        index=0,
        help="Select the active Google Gemini embedding model to index and retrieve document clauses."
    )
else:
...
# Setup visual three-panel tabs for cleaner UX
tab_chat, tab_history, tab_eval = st.tabs([
...
with tab_eval:
    st.markdown("### 📊 Ragas Automated Metrics Evaluation")
    st.markdown(
        "Evaluate the quality of your RAG pipeline against the 10-question Golden evaluation set "
        "using **Google Gemini 2.5 Flash** as the evaluator judge model."
    )
    
    # Path to evaluation results
    eval_results_file = os.path.join(WORKSPACE_DIR, "evaluate", "eval_results.json")
    
    # Run evaluation button
    if st.button("⚡ Run Full Ragas Metrics Evaluation", use_container_width=True, help="Executes the RAG queries and evaluates faithfulness, relevance, recall, and precision using Gemini."):
        with st.spinner("Executing 10 evaluation queries, retrieving document contexts, and running Ragas evaluation judge... (this takes about 1-2 minutes)"):
            try:
                import subprocess
                # Run the evaluation script
                res = subprocess.run(
                    [sys.executable, os.path.join(WORKSPACE_DIR, "evaluate", "run_evaluation.py")],
                    capture_output=True,
                    text=True,
                    check=True
                )
    ...
    if os.path.exists(eval_results_file):
        try:
            ...
            scores = eval_data.get("global_scores", {})
            
            st.markdown("#### 🎯 Global Ragas Scorecard")
            col_m1, col_m2, col_m3, col_m4 = st.columns(4)
            
            with col_m1:
                st.metric(
                    label="Faithfulness",
                    value=f"{scores.get('faithfulness', 0.0) * 100:.1f}%",
                    help="Measures if the answers generated by the LLM are strictly grounded in the retrieved document context. Higher is better."
                )
            with col_m2:
                st.metric(
                    label="Answer Relevance",
                    value=f"{scores.get('answer_relevance', 0.0) * 100:.1f}%",
                    help="Measures if the generated answers directly address the user question without redundant details. Higher is better."
                )
            with col_m3:
                st.metric(
                    label="Context Recall",
                    value=f"{scores.get('context_recall', 0.0) * 100:.1f}%",
                    help="Measures if all key facts needed to answer the question were successfully retrieved from your policies. Higher is better."
                )
            with col_m4:
                st.metric(
                    label="Context Precision",
                    value=f"{scores.get('context_precision', 0.0) * 100:.1f}%",
                    help="Measures if the retrieved clauses are highly relevant and dense, without unnecessary noise. Higher is better."
                )
                
            st.markdown("---")
            st.markdown("#### 📋 Detailed Per-Question Evaluation Breakdown")
            ...
            for item in eval_data.get("per_question_results", []):
                rows.append({
                    ...
                    "Faithfulness": f"{item['faithfulness'] * 100:.0f}%",
                    "Answer Relevance": f"{item['answer_relevance'] * 100:.0f}%",
                    "Context Recall": f"{item['context_recall'] * 100:.0f}%",
                    "Context Precision": f"{item['context_precision'] * 100:.0f}%"
                })
            df = pd.DataFrame(rows)
            st.dataframe(df, use_container_width=True, hide_index=True)
            
            # Interactive details expander for each question
            st.markdown("#### 🔍 Deep Inspection of Questions & Answers")
            for item in eval_data.get("per_question_results", []):
                with st.expander(f"❓ Q{item['id']}: \"{item['question'][:80]}...\""):
                    st.markdown(f"**Question:** {item['question']}")
                    st.markdown(f"**Ground Truth Answer:** *\"{item['ground_truth']}\"*")
                    st.markdown(f"**Generated Answer:** *\"{item['answer']}\"*")
                    
                    st.markdown("##### Scores:")
                    c1, c2, c3, c4 = st.columns(4)
                    c1.markdown(f"**Faithfulness:** `{item['faithfulness']*100:.0f}%`")
                    c2.markdown(f"**Answer Relevance:** `{item['answer_relevance']*100:.0f}%`")
                    c3.markdown(f"**Context Recall:** `{item['context_recall']*100:.0f}%`")
                    c4.markdown(f"**Context Precision:** `{item['context_precision']*100:.0f}%`")
                    
        except Exception as e:
            st.error(f"Error displaying evaluation data: {e}")
    else:
        st.info("No evaluation scores are currently available. Click the button above to run Ragas metrics against the golden set.")
